...
- The main platform model is `Salus`, `Societas`, `Terra`, with `Morbus` nested inside Salus.
...
- `src/components/earth-scene.tsx` owns the 3D scene: Physical Earth, Digital Earth, Meta Earth label, data connectors, solar orbit model, and orbit controls.
...
- `src/lib/data-index.ts` feeds `/projects/sapiens-scientia-data-index` and the Digital Earth/data index surfaces.
